    ...
    useEffect(() => {
      ...
      if (currentArtwork?.document) {
        fuderuCanvas.importDocument(currentArtwork.document).then(() => {
          ...
          console.warn("importDocument error:", err);
          syncLayers();
        });
      } else if (currentArtwork?.layers && currentArtwork.layers.length > 0) {
        const topLayer = currentArtwork.layers[currentArtwork.layers.length - 1];
        const doc = {
          version: 1 as const,
          width: canvasWidth,
          height: canvasHeight,
          layers: currentArtwork.layers.map(l => ({
            id: l.id,
            name: l.name,
            visible: l.visible,
            opacity: l.opacity,
          ...
          activeLayerId: topLayer ? topLayer.id : currentArtwork.layers[0].id
        };
        fuderuCanvas.importDocument(doc).then(() => {
          const bg = fuderuCanvas.getLayers()[0];
          if (bg && !currentArtwork.layers![0]?.dataUrl) {
          ...
          console.warn("importDocument legacy error:", err);
          syncLayers();
        });
      } else {
        const bg = fuderuCanvas.getLayers()[0];
